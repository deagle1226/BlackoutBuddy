...
package com.blackoutbuddy.android.locpoll;
...
public class LocationPollerService extends Service {
  ...
  private LocationManager locationManager;
	...
	public static final int FRESHNESS_THRESHOLD = 1000 * 60 * 5; // five minutes
	public static final int ACCURACY_DELTA = 200;
	public static final float ACCURACY_THRESHOLD = 15.0f;
	public static final int MINIMUM_UPTIME = 15 * 1000;
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
		...
		// currentBest = locationManager.getLastKnownLocation(parameters.getProviders()[1]); // gps's last known if size is 2
		currentBest = locationManager.getLastKnownLocation(parameters.getProviders()[0]); // gps's last known if size is 1
    ...
    public boolean isBetterLocation(Location location, Location currentBest) {
		...
		boolean isSignificantlyNewer = timeDelta > FRESHNESS_THRESHOLD;
		boolean isSignificantlyOlder = timeDelta < -FRESHNESS_THRESHOLD;
    ...
    protected void onPreExecute() {
		...
		private void servicePause(int length){
